...
const radioDevolverCodigo = document.getElementById('radio-devolver');
const radioCambiarNombre = document.getElementById('radio-cambiar-nombre');
const radioDevolverNombre = document.getElementById('radio-devolver-nombre');
...
inputBuscarCodigo.addEventListener('change', (event) => {
  const valor = event.target.value;
  if (valor.trim() !== '') {
  ...
  if (productos.length > 0) {
    ...
    if(radioCambiarCodigo.checked) {
      ...
      swalWithButtons.fire({
        ...
        ipcRenderer.send('guardar-cambio', producto);
        ipcRenderer.send('actualizar-ventana');
      });
    } else if(radioDevolverCodigo.checked) {
      ipcRenderer.send('guardar-devolucion', producto);
...
btnAgregarNombre.addEventListener('click', () => {
  ...
  } else if(radioDevolverNombre.checked) {
    ipcRenderer.send('guardar-devolucion', producto);
    ipcRenderer.send('abrir-ventana-devolver', total);
  } else {
    const swalWithButtons = Swal.mixin({
...
btnCancelarNombre.addEventListener('click', () => {
    ...
    radioDevolverNombre.checked = false;
});
...
btnCancelarCodigo.addEventListener('click', () => {
  ...
  radioDevolverCodigo.checked = false;
});
