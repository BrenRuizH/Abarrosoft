...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const labelTotales = document.getElementById('total-compras');
  ...
  btnGenerar.addEventListener('click', () => {
    ...
    ipcRenderer.send('get-all-compras', fechaInicio, fechaFin);
    ipcRenderer.send('get-total-compras', fechaInicio, fechaFin);
  });

  // Manejar la respuesta del proceso principal con el historial cargado
ipcRenderer.on('all-compras-loaded', (event, compras) => {
...
// Escuchar el evento 'compras-loaded' desde el proceso de renderizado
ipcRenderer.on('compras-loaded', (event, compras) => {
  console.log('Received compras:', compras); // Add this line for debugging

  if (typeof compras === 'number') {
    // If 'compras' is a valid number, format it and update the label
    const formattedCompras = compras.toFixed(2);
    labelTotales.textContent = 'TOTAL COMPRAS: $' + formattedCompras;
  } else {
    // Handle the case where 'compras' is not a valid number
    labelTotales.textContent = 'TOTAL COMPRAS: N/A'; // Display an error message or placeholder
  }
});
