...
ipcRenderer.on('productos-loaded', (event, paginatedProducts, totalRecords) => {
  ...
  paginatedProducts.forEach((producto) => {
    ...
    row.innerHTML = `
      ...
      <td>
        <button id="editar-button" class="btn btn-success btn-sm" data-id="${producto.id}">
...
document.getElementById('productos-table-container').addEventListener('click', (event) => {
  if (event.target.classList.contains('btn-success')) {
    ...
    ipcRenderer.send('open-productos-form', productoId, currentPage);
  }
});
...
// Manejar el evento de agregar cantidad del producto
document.getElementById('productos-table-container').addEventListener('click', (event) => {
